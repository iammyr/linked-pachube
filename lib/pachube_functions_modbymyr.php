...
class Pachube
{
	...
	private $Api;
	private $base_url = "http://api.pachube.com/";



	function __construct ($api, $version)
	{
		$this->Api = $api;
	...
	public function datapoints($feed_id, $datastream_id, $type = '', $start='', $end='', $interval=''){
		$url = $this->base_url . "feeds/".$feed_id . "/datastreams/" . $datastream_id . "/datapoints";
		if (empty($type)){
			$type = "json";
		}
		
		$data = json_decode($this->retrieveData($url, $type, $start, $end, $interval), true);
		//print_r ($data);
	...
	public function retrieveData ( $url='', $type='' , $start='', $end='', $interval='')
	{
		if(empty($this->Api))
		{
			//trigger_error(999);
	...
	public function getLatLon ( $search='')
	{
		if (!empty($search))
		{
			if(function_exists("curl_init"))
			{
				...
				$url = $this->base_url . "feeds?$search";
				...
				array_push($reported_locations, $json_data["totalResults"]);
	...
	/**
	 *
	 * time must be ISO 8601 (example: 2010-08-02T14:01:46Z)
	 * @param unknown_type $feed_id
	 * @param unknown_type $start
	 * @param unknown_type $end
	 * @param unknown_type $interval
	 */
	public function environment_historical($feed_id, $start, $end, $interval)
	{
		$data = json_decode($this->retrieveData( "feeds/$feed_id", "json", $start, $end, $interval), true);
		//print_r ($data);
		//		$return_data = (strcmp($data,"Unable to find specified resource.")==0) ? json_decode("{\"location\":{}\"datastreams\":[{}]}") : $data;
		$return_data = $data;
		return $return_data;
	...
	{
		$data = json_decode($this->retrieveData ( "feeds/$feed_id", "json" ), true);
		//print_r ($data);
		//		$return_data = (strcmp($data,"Unable to find specified resource.")==0) ? json_decode("{\"location\":{}\"datastreams\":[{}]}") : $data;
		$return_data = $data;
		return $return_data;
